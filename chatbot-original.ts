...
if (!apiKey) {
  console.error(
    "Error: ANTHROPIC_API_KEY environment variable is not set. Please set it in .env.local"
  );
  process.exit(1);
}
...
const tools: Anthropic.Tool[] = [
  ...
  },
  {
    name: "weather",
    description:
      "Get the current weather for a specific location. Use this when the user asks about weather conditions.",
    input_schema: {
      type: "object",
      properties: {
        location: {
...
async function getWeather(location: string): Promise<string> {
  try {
    ...
    const current = (
      data as { current_condition: { temp_F: number; weatherDesc: { value: string }[] }[] }
    ).current_condition[0];
