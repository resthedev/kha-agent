...
import { Chunk, KnowledgeBaseIndex } from "./types";
...
export async function buildOrUpdateIndex(forceRebuild = false): Promise<KnowledgeBaseIndex> {
  ...
  if (existingIndex && existingIndex.embeddingModel === embeddingModel) {
    ...
    for (const filePath of markdownFiles) {
      ...
      const hasChanged = existingIndex.fileHashes[filePath] !== currentHashes[filePath];
  ...
  console.log(
    `Processing ${filesToProcess.length} file(s), keeping ${unchangedChunks.length} unchanged chunk(s)`
  );
