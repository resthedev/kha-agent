...
export async function POST(req: Request) {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7243/ingest/2727ac65-d4c4-4377-a3ee-e2be505a8a5e', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'route.ts:14', message: 'sending to Claude', data: { hasSystemPrompt: !!ragSystemPrompt, systemPromptLength: ragSystemPrompt?.length || 0, lastUserMsg: messages[messages.length - 1]?.content?.substring(0, 100) }, timestamp: Date.now(), sessionId: 'debug-session', hypothesisId: 'C' }) }).catch(() => { });
    // #endregion
